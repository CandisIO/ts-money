...
describe('Money', function () {
  ...
  it('should create a new instance from decimal using `.fromDecimal()` even if too many decimal places if rounder function provided', function () {
    ...
    var money1 = Money.fromDecimal(
      { amount: 10.01, currency: 'EUR' },
      Math.ceil
    );
  ...
  it('should compare correctly', function () {
    ...
    expect(subject.greaterThanOrEqual(new Money(1500, Money.EUR))).to.equal(
      false
    );
    expect(subject.greaterThanOrEqual(new Money(500, Money.EUR))).to.equal(
      true
    );
    expect(subject.greaterThanOrEqual(new Money(1000, Money.EUR))).to.equal(
      true
    );
